...
namespace VR_Soft
{
	...
	class VRSOFT_DLL IEvent : public CSingleton<IEvent>
	{
	public:
		virtual ~IEvent(void) { }
	...
	public:
		...
		// 获得鼠标是否单击
		virtual bool GetMouseButton(const MouseSymbol mouseSymbol) const = 0;
		// 获得是否安装键盘
		virtual bool GetKey(const KeySymbol keySymbol) const = 0;
		// 获得鼠标的二维位置
		virtual osg::Vec2d GetMousePosition2D(void) const = 0;
		// 设置鼠标
		virtual void SetMouseButton(const MouseSymbol mouseSymbol) = 0;
		// 设置是否安装键盘
		virtual void SetKey(const KeySymbol keySymbol) = 0;
		// 设置点坐标
		virtual void SetMousePosition2D(const osg::Vec2d vPos) = 0;
	};
