#include "OceanEarthPPU.h"

#include "ScreenAlignedQuad.h"

namespace VR_Soft
{
	COceanEarthPPU *COceanEarthPPU::Instance()
	{
		static COceanEarthPPU instance;

		return &instance;
	}

	COceanEarthPPU::COceanEarthPPU(void)
	{
	}
	...
	//name:名称需要唯一
	osg::Texture* COceanEarthPPU::createTexture( std::string type, 
	...
	osg::Camera* COceanEarthPPU::createPass( PassType type, std::string name, bool bQuad )
	{
	...
	//输出至绑定纹理
	void COceanEarthPPU::renderToTexture( osg::Camera *camera,
	...
	void COceanEarthPPU::renderToTexture2DArray( osg::Camera *camera,
	...
	void COceanEarthPPU::multiRenderToTexture(
		osg::Camera *camera,
	...
	//输入纹理至相机状态机
	void COceanEarthPPU::renderForBindTexture( osg::StateSet* stateset, std::string name, osg::Texture* textureBuffer, int unit )
	{
	...
	//最终输出，需将相机状态恢复
	void COceanEarthPPU::renderForOutput( osg::Camera *camera, double nW, double nH )
	{
	...
	osg::Geode* COceanEarthPPU::createScreenQuad( osg::Vec3f corner, osg::Vec2f dims, osg::Vec2s textureSize )
	{
	...
	bool COceanEarthPPU::removePass( const std::string& name )
	{
	...
	bool COceanEarthPPU::getPassData( const std::string& name, PassData& data )
	{
		PassList& passList = getPassList();
	...
	bool COceanEarthPPU::setPassActivated( const std::string& name, bool activated )
	{
		PassList& passList = getPassList();
	...
	bool COceanEarthPPU::getPassActivated( const std::string& name )
	{
		PassList& passList = getPassList();
	...
	bool COceanEarthPPU::setPassIndex( const std::string& name, unsigned int index )
	{
	...
	unsigned int COceanEarthPPU::getPassIndex( const std::string& name )
	{
		PassList& passList = getPassList();
	...
	bool COceanEarthPPU::setTexture( const std::string& name, osg::Texture* tex )
	{
	...
	bool COceanEarthPPU::removeTexture( const std::string& name )
	{
		TextureMap::iterator itr = _textureMap.find(name);
	...
	osg::Texture* COceanEarthPPU::getTexture( const std::string& name )
	{
		TextureMap::iterator itr = _textureMap.find(name);
	...
	const osg::Texture* COceanEarthPPU::getTexture( const std::string& name ) const
	{
	...
	bool COceanEarthPPU::setProgram( const std::string& name, osg::Program* pgm )
	{
	...
	bool COceanEarthPPU::removeProgram( const std::string& name )
	{
		ProgramMap::iterator itr = _programMap.find(name);
	...
	osg::Program* COceanEarthPPU::getProgram( const std::string& name )
	{
		ProgramMap::iterator itr = _programMap.find(name);
	...
	const osg::Program* COceanEarthPPU::getProgram( const std::string& name ) const
	{
	...
	bool COceanEarthPPU::setOEStateSet( const std::string& name, osg::StateSet* ss )
	{
	...
	bool COceanEarthPPU::removeOEStateSet( const std::string& name )
	{
		StateSetMap::iterator itr = _stateSetMap.find(name);
	...
	osg::StateSet* COceanEarthPPU::getOEStateSet( const std::string& name )
	{
		StateSetMap::iterator itr = _stateSetMap.find(name);
	...
	osg::Shader* COceanEarthPPU::createShader( std::string type, std::string name, 
	...
	char *COceanEarthPPU::textFileRead(char *fn) 
	{
		FILE *fp;
		char *content = NULL;
		int count = 0;
	...
	osg::Program* COceanEarthPPU::createProgram( std::string name, 
