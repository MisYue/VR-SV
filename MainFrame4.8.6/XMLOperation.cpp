...
bool XMLOperation::SaveXMLFile(const TargetAreaParameter &_pT,const LGRockParameter &_pLG)
{
	...
	QDomAttr ShockWavePressure = doc.createAttribute(QString("冲击波超压判据"));
	...
	if (count == 0)
	{
		...
		ShockWavePressure.setValue(str.setNum(_pLG.ShockWavePressure));
		...
		RockParameter.setAttributeNode(ShockWavePressure);
			...
			for(int i=0; i<list.count(); i++)
			{
				...
				if(node.isElement())
				{
					...
					else if(_pE.tagName() == QString::fromLocal8Bit("弹体属性"))
					{
						...
						_RockObject.Frag_FragmentDamageCriterion = _pE.attribute("毁伤判据").toDouble();
						_RockObject.ShockWavePressure = _pE.attribute("冲击波超压判据").toDouble();
						...
						for (int i = 0; i < FallList.size()-1;++i)
						{
...
void XMLOperation::RockTable()
{
	if (!RockVector.empty())
	{
		...
		for (;it_tor != RockVector.end();++it_tor)
		{
			...
			_pRockTab->setItem(TRow,27,new QTableWidgetItem(str.setNum(it_tor->ShockWavePressure)));
			_pRockTab->setItem(TRow,28,new QTableWidgetItem(str.setNum(it_tor->InterpolationNumber)));
			...
			_pRockTab->setItem(TRow,30,new QTableWidgetItem(str));
			_pRockTab->setItem(TRow,31,new QTableWidgetItem(_xmlFileUnity.at(TRow)));
			++TRow;
		}
...
void XMLOperation::GetRatioChangeRock(LGRockParameter &LRock,const Bit_UnitSystem *pV)
{
	...
	p_war.Set_Initial(1,*pV,LRock.ShellMass_Clylinder,LRock.ChargeMass_Clylinder,LRock.AllMass_Whole,LRock.ChargeMass_Whole,1,1,
	...
	LRock.Frag_Mass = LRock.Frag_Mass*_p->Get_MassCoefficient()/_LocalUnity->Get_MassCoefficient();
